...
La même étude fixe déjà une orientation. Le cerveau de la supervision quitte le siège. Il migre dans le nuage. De petits programmes, les agents, demeurent sur les postes. Un réseau privé relie l’ensemble. Le présent chapitre convertit cette orientation en solution. Notre apport pour l’entreprise se situe ici.

Précisons le vocabulaire avant d’avancer. Un *Security Information and Event Management* (SIEM) rassemble les traces, les compare, puis lève une alerte. Un agent, logiciel léger, habite le poste et envoie ces traces. Une réponse active exécute, sur la machine visée, un geste automatique : bloquer une adresse, ôter un fichier, isoler l’hôte. Le *File Integrity Monitoring* (FIM) surveille les changements de fichiers. Le *Mean Time To Detect* (MTTD) mesure, en secondes ou en semaines, le délai moyen avant découverte. Le *Mean Time To Respond* (MTTR) mesure le délai moyen avant correction.
...
Le serveur central survit à une panne du siège. Une coupure à Yaoundé n’aveugle plus Maroua. D’où le nuage pour le cerveau, et les postes pour les agents.
...
Deux conséquences suivent. Premièrement, le serveur d’alertes n’écoute pas sur Internet. Un inconnu ne s’y enregistre pas comme s’il était un poste de SSN. Deuxièmement, une panne électrique au siège n’éteint plus la vue sur Maroua. Le cerveau habite le nuage. Le chapitre 2 avait nommé ce risque. La solution le traite.
...
Le deuxième temps crée, dans le nuage, l’ordinateur de supervision et ses règles d’accès. Il en tire l’adresse utile à l’étape suivante.
...
| Panne au siège               | Aveugle aussi Maroua          | Cerveau resté dans le nuage  |
+------------------------------+-------------------------------+------------------------------+
```
